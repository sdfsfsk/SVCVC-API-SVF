...
class BSRoformer(Module):
    ...
    def forward(
        ...
        x_needs_amd_stft_workaround = os.environ.get("MSST_FORCE_CPU_STFT") == "1"
        try:
            _dev_name = torch.cuda.get_device_name(device) if device.type == "cuda" else ""
            if os.environ.get("RVC_NATIVE_ROCM") != "1" and ("ZLUDA" in _dev_name or "AMD" in _dev_name):
                x_needs_amd_stft_workaround = True
        except Exception:
            pass
        ...
        if x_needs_amd_stft_workaround:
