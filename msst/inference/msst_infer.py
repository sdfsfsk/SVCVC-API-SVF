...
class MSSeparator:
    ...
    def load_model(self):
        ...
        self.logger.info(
            f"Separator params: model_type: {self.model_type}, model_path: {self.model_path}, config_path: {self.config_path}, output_folder: {self.store_dirs}")
        self.logger.info(
            f"Audio params: output_format: {self.output_format}, audio_params: {self.audio_params}")
        self.logger.info(
    ...
    def process_folder(self, input_folder):
        ...
        self.logger.info(
            f"Input_folder: {input_folder}, Total files found: {len(all_mixtures_path)}, Use sample rate: {sample_rate}")
            ...
            for instr in results.keys():
                ...
                if save_dir and type(save_dir) == str:
                    ...
                    for dir in save_dir:
                        ...
                        self.logger.debug(
                            f"Saved {instr} for {file_name}_{instr}.{self.output_format} in {dir}")
    ...
    def separate(self, mix):
        ...
        elif isstereo and len(mix.shape) != 1 and mix.shape[0] > 2:
            mix = np.mean(mix, axis=0)
            mix = np.stack([mix, mix], axis=0)
            self.logger.warning(
                f"Track has more than 2 channels, taking mean of all channels and adding a second channel.")
        elif not isstereo and len(mix.shape) != 1:
            mix = np.mean(mix, axis=0)
            self.logger.warning(
                f"Track has more than 1 channels, but model is mono, taking mean of all channels.")
        ...
        if self.config.training.target_instrument is not None:
            ...
            other_instruments = [instr for instr in self.config.training.instruments if
                                 instr != target_instrument]
    ...
    def update_inference_params(self, config, params):
        for key, value in {
            ...
            if params.get(key) is not None:
                config[value][key] = int(params[key]) if key != 'normalize' else bool(params[key])
        return config
